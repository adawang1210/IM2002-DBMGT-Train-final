...
### 範本 H：根據 Code Review 報告修復程式碼

> **使用時機：** 當隊友或學長在 GitHub PR 上給了一份詳細的 Code Review 報告（包含 Bug、規格偏離、細節建議），你需要請 AI 幫你一次性、安全地修復所有問題時。

```text
我收到了一份針對我程式碼的 Code Review 報告。請幫我根據報告中的建議，一次性修復所有問題，並輸出完整的最新程式碼。

【嚴格規則】
1. 針對報告中提到的每一個 Issue 和 Bug，請在修改的程式碼上方加上簡短的註解（例如：`# 修正 Issue #2：對齊 Docstring 規格`），方便 Reviewer 複查。
2. 絕對不要修改報告中「沒有提到」的功能或邏輯，保持未受影響的部分原封不動。
3. 確保修改後的程式碼符合原有的架構慣例，且無語法錯誤。

【要修改的原始程式碼】
[貼上你目前的完整程式碼]

【Code Review 報告全文】
[貼上學長或隊友給的 Markdown 審查報告]
